...
export async function POST(request: NextRequest) {
  try {
    ...
    const emailContent = isStaffEmail(email) ? staffWelcome(userName) : welcome(userName);
    ...
    sendCustomEmail(email, emailContent)
      .then((r) => {
        if (!r.success) {
          logger.warn('Failed to send welcome email after verification (resolved)', {
            error: r.error,
            email,
          });
        }
      ...
      .catch((err) => {
        logger.warn('Failed to send welcome email after verification (rejected)', {
          error: err,
          email,
        });
      });
