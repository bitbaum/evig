...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!rateLimitResult.allowed) {
      ...
      return apiError(null, 'Zu viele Einreichungen. Bitte versuche es später erneut.', 429);
    }
    ...
    if (data.category) {
      const [category] = await db
        ...
        .from(blogCategories)
        .where(or(eq(blogCategories.name, data.category), eq(blogCategories.slug, data.category)));
