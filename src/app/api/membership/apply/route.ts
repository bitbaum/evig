...
export async function POST(request: NextRequest) {
  try {
    ...
    type ApplyResult = { kind: 'blocked'; message: string } | { kind: 'ok'; applicationId: string };
    ...
    const fee =
      result.data.memberType === 'reduced' ? MEMBERSHIP.fees.reduced : MEMBERSHIP.fees.regular;
    ...
    sendCustomEmail(result.data.applicantEmail, {
      ...
      logger.error('Failed to send membership welcome email', {
        err,
        applicationId: application.id,
      }),
    );
