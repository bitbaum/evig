...
export function ListingCard({ listing, variant = 'default', className = '' }: ListingCardProps) {
  ...
  return (
    ...
    <div
      className={`group card-shell relative overflow-hidden hover:border-strong transition-colors ${className}`}
    >
      <Link
      ...
      >
        ...
        <div className={isCompact ? 'p-3' : 'p-4'}>
          ...
          <h3
            className={`font-semibold text-text-primary line-clamp-2 ${isCompact ? 'text-sm' : 'text-base'}`}
          >
            {listing.title}
          </h3>

          <div
            className={`mt-3 font-mono tabular-nums ${isCompact ? 'text-base' : 'text-lg'} font-semibold text-text-primary`}
          >
...
export function ListingCardGrid({
  ...
  return <div className={`${gridClass} gap-4 md:gap-6 ${className}`}>{children}</div>;
}
