...
export function TimecardReviewDrawer({
  ...
  const reportHref = card
    ? `/admin/team/report/${card.user_id}/${card.period_start.slice(0, 7)}`
    : null;
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      >
                    ...
                    <td className="py-1.5 text-right tabular-nums">
                      {editing ? (
                        <Input
                          ...
                          onChange={(ev) =>
                            patchEntry(i, {
                              duration_minutes: Math.round((parseFloat(ev.target.value) || 0) * 60),
                            })
                          }
        ...
        <div className="space-y-3 border-t border-subtle px-5 py-4">
          {isOwnCard && (
            <p className="text-sm text-warning-700 dark:text-warning-400">{t('drawerOwnNotice')}</p>
          )}
